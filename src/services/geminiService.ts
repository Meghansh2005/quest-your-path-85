...
const API_KEY = 'AIzaSyBiLWamOyLjHSsVXbcstJOCZXUSvlQCOPw';
...
class GeminiService {
  ...
  async generateWorkplaceScenario(
    ...
    const context = this.conversationHistory.join('\n');
    const prompt = `You are a senior career coach and industry expert specializing in ${fieldOfInterest}. Create a highly realistic, field-specific workplace scenario that reflects actual challenges professionals face in ${fieldOfInterest}.

FIELD: ${fieldOfInterest}
USER: ${userBackground.name}
PREVIOUS SCENARIOS TO AVOID: ${previousScenarios.join('; ')}

CREATE A FIELD-SPECIFIC SCENARIO THAT:
1. Reflects real ${fieldOfInterest} industry challenges and terminology
2. Tests decision-making skills relevant to ${fieldOfInterest} professionals
3. Reveals leadership potential in ${fieldOfInterest} contexts
4. Shows problem-solving approach using ${fieldOfInterest} methodologies
5. Demonstrates industry-specific knowledge and best practices
6. Assesses communication style relevant to ${fieldOfInterest} stakeholders
...
Return ONLY this JSON structure:
{
  ...
  private getFallbackScenarioAnalysis(field: string) {
    ...
    return {
      personalityProfile: fieldAnalysis.personalityProfile || [],
      workStylePreferences: fieldAnalysis.workStylePreferences || [],
      leadershipStyle: fieldAnalysis.leadershipStyle || `${field} leadership style`,
      problemSolvingApproach: fieldAnalysis.problemSolvingApproach || `${field} problem-solving approach`,
      careerRecommendations: fieldAnalysis.careerRecommendations || [],
      developmentAreas: fieldAnalysis.developmentAreas || [],
      overallScore: 85,
      topStrengths: [
        `Strong ${field} expertise`,
        fieldAnalysis.leadershipStyle || 'Effective leadership approach',
        fieldAnalysis.problemSolvingApproach || 'Systematic problem-solving',
...
[
  ...
  },
  {
    ...
    "prerequisites": "Completion of foundational certificate"
      ...
      return learningPath.map((item: any) => ({
...
[
  ...
  },
  {
    ...
    "workplaceExamples": ["Facilitates effective meetings", "Builds strong team relationships", "Handles conflicts diplomatically"]
      ...
      const result = await model.generateContent(prompt);
      const response = await result.response;
      const personalityTraits = JSON.parse(response.text());
...
Ensure the scenario:
- Uses authentic ${fieldOfInterest} terminology and concepts
- Reflects current industry trends and challenges
- Includes stakeholders and situations specific to ${fieldOfInterest}
- Tests knowledge of ${fieldOfInterest} best practices
- Presents realistic options that ${fieldOfInterest} professionals would face

Return ONLY this JSON structure:
{
  "scenario": "${fieldOfInterest}-specific scenario title using industry terminology",
  "context": "Detailed background situation (2-3 sentences) using authentic ${fieldOfInterest} context, stakeholders, and challenges",
  "challenge": "The specific ${fieldOfInterest} problem or decision that requires industry knowledge (1-2 sentences)",
  "options": [
    {
      "id": "option_1",
      "text": "First response option reflecting ${fieldOfInterest} best practices and methodology",
      "skills": ["${fieldOfInterest}-specific skill 1", "${fieldOfInterest}-specific skill 2", "${fieldOfInterest}-specific skill 3"],
      "personality": ["trait1", "trait2"]
    },
    {
      "id": "option_2", 
      "text": "Second response option showing different ${fieldOfInterest} approach",
      "skills": ["skill1", "skill2"],
      "personality": ["trait1", "trait2"]
    },
    {
      "id": "option_3",
      "text": "Third response option demonstrating alternative ${fieldOfInterest} strategy",
      "skills": ["skill1", "skill2"],
      "personality": ["trait1", "trait2"]
    },
    {
      "id": "option_4",
      "text": "Fourth response option showing innovative ${fieldOfInterest} solution",
      "skills": ["skill1", "skill2"], 
      "personality": ["trait1", "trait2"]
    }
  ],
  "followUpQuestions": ["${fieldOfInterest}-specific reflection question 1", "${fieldOfInterest}-specific reflection question 2"]
}`;
...
Return ONLY this JSON structure:
{
  "personalityProfile": [
    ...
    },
    {
      ...
      const instance = new GeminiService();
      const result = await instance['makeStructuredRequest'](prompt);
